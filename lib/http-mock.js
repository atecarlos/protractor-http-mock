	...
	newModule.config(['$provide', function($provide){
		$provide.decorator('$http', ['$delegate', '$q', function($http, $q) {
			...
			function objectMatch(expectationObject, configObject){
				var match = !expectationObject && !configObject;

				if(!match && expectationObject && configObject){
					console.log('object matching', expectationObject, configObject)
					match = true;
			...
			function match(config, expectationRequest){
				console.log(expectationRequest, config);
				
				return	expectationRequest.method === config.method &&
						endsWith(config.url, expectationRequest.path) &&
						(objectMatch(expectationRequest.params, config.params)) &&
						(objectMatch(expectationRequest.data, config.data));
			}
